...
import "./ArborSwapDeps/IArborSwapLock.sol";
import "./ArborSwapDeps/IUniswapV2Router02.sol";
import "./ArborSwapDeps/IUniswapV2Pair.sol";
import "./ArborSwapDeps/IUniswapV2Factory.sol";
import "./ArborSwapDeps/FullMath.sol";

contract ArborSwapLock is IArborSwapLock, Ownable {
    ...
    uint256 public feeNormalLock;
    uint256 public feeVestingLock;
    ...
    event LogUpdateFeeAddr(address _feeAddr);
    event LogUpdateFeeNormal(uint256 _fee);
    event LogUpdateFeeVesting(uint256 _fee);
    ...
    constructor(
        ...
        feeAddr = _feeAddr;
        feeNormalLock = _feeNormal;
        feeVestingLock = _feeVesting;
    }
    ...
    function lock(
        ...
        require(msg.sender.balance >= feeNormalLock, "Incuficient balance");
        require(
            msg.value >= feeNormalLock,
            "Should be more or equal than current fee"
        );
        id = _createLock(
            owner,
            token,
    ...
    function multipleVestingLock(
        ...
        require(
            msg.sender.balance >= feeVestingLock * owners.length,
            "Incuficient balance"
        );
        require(
            msg.value >= feeVestingLock * owners.length,
            "Should be more or equal than current fee * number of locks"
        );
        feeAddr.transfer(msg.value);
        return
    ...
    function updateFeeAddr(address payable newFeeAddr) external onlyOwner {
        require(newFeeAddr != feeAddr, "Already set to this value");
        require(newFeeAddr != address(0), "Invalid address");
        feeAddr = newFeeAddr;
        emit LogUpdateFeeAddr(newFeeAddr);
    }

    function updateFeeNormal(uint256 newFee) external onlyOwner {
        require(newFee != feeNormalLock, "Already set to this value");
        feeNormalLock = newFee;
        emit LogUpdateFeeNormal(newFee);
    }

    function updateFeeVesting(uint256 newFee) external onlyOwner {
        require(newFee != feeVestingLock, "Already set to this value");
        feeVestingLock = newFee;
        emit LogUpdateFeeVesting(newFee);
    }
}
